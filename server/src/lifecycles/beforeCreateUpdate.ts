...
const beforeCreateUpdateLifecycle = async (strapi: Core.Strapi) => {
  ...
  const beforeCreateUpdate = async (event) => {
    ...
    if (!isAvailable) {
      throw new errors.ValidationError('Permalink value must be unique.');
    }
  };
...
export default beforeCreateUpdateLifecycle;
